...
def longest_palindromic_subsequence(s):
    ...
    for cl in range(2, n + 1):
        for i in range(n - cl + 1):
            j = i + cl - 1
            # If the end characters match, increment the LPS length by 2,
            # considering the LPS of the substring between them
            if s[i] == s[j]:
                dp[i][j] = dp[i+1][j-1] + 2
            # If the end characters don't match, take the maximum LPS length
            # between excluding the first character or excluding the last character
            else:
                dp[i][j] = max(dp[i][j-1], dp[i+1][j])

    # The top-right cell of the DP table contains the LPS length for the entire string
    return dp[0][n-1]
