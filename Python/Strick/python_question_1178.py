...
*   int add(int val) Appends the integer val to the stream and returns the element representing the kth largest element in the stream. If the stream has less than k elements, return -1.
...
class KthLargest:
    def __init__(self, k: int, nums: list[int]):
        ...
        for num in nums:
            self.add(num) # initially populate the heap
    ...
    def add(self, val: int) -> int:
        ...
        Args:
            val: The integer to append to the stream.

        Returns:
            The kth largest element in the stream, or -1 if the stream has less than k elements.
        """
        if len(self.heap) < self.k:
            heapq.heappush(self.heap, val)  # Add to the heap if it has less than k elements
        ...
        if len(self.heap) < self.k:
            ...
            return self.heap[0] # Return the kth largest element (root of the min-heap)
