...
class KthLargest:
    def __init__(self, k: int, nums: list[int]):
        ...
        while len(self.heap) > k:
            heapq.heappop(self.heap)  # Remove elements until only k elements remain
    ...
    def add(self, val: int) -> int:
        ...
        if len(self.heap) < self.k:
            heapq.heappush(self.heap, val)  # Add the new element to the heap if it has less than k elements
