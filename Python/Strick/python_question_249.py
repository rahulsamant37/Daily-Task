    ...
    for l in range(2, n + 1):
        for i in range(n - l + 1):
            ...
            if s[i] == s[j]:
                ...
                # then the LPS length is 2 + LPS length of the substring without these two characters
                dp[i][j] = 2 + (dp[i + 1][j - 1] if (i + 1 <= j - 1) else 0)
            else:
                # If the first and last characters are different,
                # then the LPS length is the maximum of LPS length of the substring without the first character
