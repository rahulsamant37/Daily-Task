...
def longest_palindromic_subsequence(s):
    ...
    # Initialize the diagonal elements of the DP table to 1, as single characters are palindromes of length 1
    for i in range(n):
        dp[i][i] = 1
    ...
    for length in range(2, n + 1):
        ...
        for i in range(n - length + 1):
            ...
            # longest palindromic subsequence of the substring without these two characters
            if s[i] == s[j]:
                dp[i][j] = 2 + dp[i + 1][j - 1]
            # Otherwise, the length of the longest palindromic subsequence is the maximum of the
            # lengths of the longest palindromic subsequences of the substrings without the first character
