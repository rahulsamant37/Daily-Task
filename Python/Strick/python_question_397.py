...
'''
Design a class to find the kth largest element in a stream.  You will be given an integer k and an initial array of integers nums.  The class should have two methods:

1.  `__init__(self, k: int, nums: List[int])`: Initializes the object with the integer k and the stream of integers nums.
2.  `add(self, val: int) -> int`: Appends the integer val to the stream and returns the kth largest element in the current stream.  If the stream has less than k elements, return the smallest element currently in the stream.
...
class KthLargest:
    ...
    def add(self, val: int) -> int:
        ...
        if len(self.heap) < self.k:
            ...
            heapq.heapreplace(self.heap, val)  # Efficiently replace smallest with val if val is larger

        if len(self.heap) < self.k:
            return min(self.heap) if self.heap else -float('inf') #Return smallest element if less than k elements
        else:
