...
def group_anagrams(strs):
    ...
    """
    anagram_map = {}  # Dictionary to store anagrams, key is the sorted string, value is the list of anagrams
    for s in strs:
        sorted_s = "".join(sorted(s))  # Sort the string to create a unique key for anagrams
        if sorted_s in anagram_map:
            anagram_map[sorted_s].append(s)  # If the key exists, append the string to the list
        else:
            anagram_map[sorted_s] = [s]  # If the key doesn't exist, create a new list with the string
    return list(anagram_map.values())  # Return the list of values (lists of anagrams)
