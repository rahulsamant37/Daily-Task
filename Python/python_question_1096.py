...
*   `KthLargest(int k, int[] nums)` Initializes the object with the integer `k` and the stream's initial elements `nums`.
...
class KthLargest:
    ...
    def __init__(self, k: int, nums: list[int]):
        ...
        Args:
            k: The desired k-th largest element.
    ...
    def add(self, val: int) -> int:
        ...
        elif val > self.heap[0]:  # val is larger than the current k-th largest element
            heapq.heapreplace(self.heap, val)  # Efficiently replace the smallest element with the new value
