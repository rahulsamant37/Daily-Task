...
def largestRectangleArea(heights):
    """
    Calculates the largest rectangular area possible in a given histogram represented by an array of heights.

    Args:
        heights: A list of integers representing the heights of the bars in the histogram.

    Returns:
        The maximum rectangular area found in the histogram.
    """
    ...
    for i in range(n + 1):
        # Iterate through the heights array, adding a dummy height of 0 at the end
        ...
        while stack and (i == n or heights[stack[-1]] > heights[i]):
            ...
            height = heights[stack.pop()]  # Pop the top bar from the stack
            width = i - stack[-1] - 1 if stack else i  # Calculate the width of the rectangle
            # If stack is empty, width is i, otherwise it's the distance to the next smaller bar on the left (or the beginning)
