...
def longest_palindrome(s):
    ...
    The approach uses dynamic programming. A 2D table dp[i][j] is created
    where dp[i][j] is True if the substring s[i:j+1] is a palindrome, and False otherwise.

    The algorithm iterates through the string, filling the dp table.
    Base cases:
    - Single characters are palindromes (dp[i][i] = True).
    - Two adjacent characters are palindromes if they are equal (dp[i][i+1] = True if s[i] == s[i+1]).

    For longer substrings, dp[i][j] is True if s[i] == s[j] and dp[i+1][j-1] is True.
    The longest palindrome is tracked during the process.
    """
    n = len(s)
    if n < 2:
        return s
