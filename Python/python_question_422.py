...
class KthLargest:
    ...
    def __init__(self, k: int, nums: list[int]):
        ...
        Args:
            k: The integer k representing the kth largest element.
            nums: The initial stream array.
        """
        self.k = k
    ...
    def add(self, val: int) -> int:
        ...
        Args:
            val: The integer to be added to the stream.

        Returns:
            The kth largest element in the stream.
        """
        # If the heap is smaller than k, add the new element
        if len(self.heap) < self.k:
            heapq.heappush(self.heap, val)
        # If the new element is larger than the smallest element in the heap, replace the smallest element
        elif val > self.heap[0]:
            heapq.heapreplace(self.heap, val)  # More efficient than pop and push when the heap is full
