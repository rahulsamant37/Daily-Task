...
def longest_palindromic_subsequence(s):
    ...
    for length in range(2, n + 1):
        ...
        for i in range(n - length + 1):
            ...
            # If the characters at the start and end are equal,
            # then the longest palindromic subsequence includes these characters,
            # and we add 2 to the length of the longest palindromic subsequence of the substring s[i+1:j]
            if s[i] == s[j]:
                dp[i][j] = dp[i + 1][j - 1] + 2
            # Otherwise, the longest palindromic subsequence is either the longest palindromic
            # subsequence of s[i+1:j] or the longest palindromic subsequence of s[i:j-1], whichever is longer
            else:
...
def test_longest_palindromic_subsequence():
    ...
    assert longest_palindromic_subsequence("aabaa") == 5
