...
def longest_palindromic_subsequence(s):
    ...
    for length in range(2, n + 1):
        for i in range(n - length + 1):
            ...
            # the length of the LPS is 2 + length of LPS of the substring without those characters
            if s[i] == s[j]:
                dp[i][j] = 2 + dp[i + 1][j - 1]

            # Otherwise, the length of the LPS is the maximum of the lengths of LPS
            # of the substrings obtained by removing either the first or last character
            else:
