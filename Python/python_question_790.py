...
def solution():
    def longest_palindromic_subsequence(s):
        ...
        # Iterate through lengths of substrings from 2 to n
        for length in range(2, n + 1):
            for i in range(n - length + 1):
                ...
                if s[i] == s[j]:
                    dp[i][j] = 2 + dp[i + 1][j - 1]
                # Otherwise, the LPS is the maximum of the LPS of the substring
                # excluding the first character and the LPS of the substring excluding the last character
                else:
