...
def longest_palindromic_subsequence(s):
    ...
    for cl in range(2, n + 1):  # cl: current length of the substring
        for i in range(n - cl + 1):
            ...
            if s[i] == s[j]:
                dp[i][j] = dp[i + 1][j - 1] + 2  # Add 2 to the length of the LPS of the inner substring
            else:
                dp[i][j] = max(dp[i][j - 1], dp[i + 1][j])  # Take the maximum of the LPS lengths of the substrings without the first or last character
