...
def encode_pdf(path, chunk_size=1000, chunk_overlap=200):
    """
    Encodes a PDF book into a vector store using Google Gemini embeddings.
...
def encode_from_string(content, chunk_size=1000, chunk_overlap=200):
    """
    Encodes a string into a vector store using Google Gemini embeddings.
...
def retrieve_context_per_question(question, chunks_query_retriever):
    ...
    Args:
        ...
        chunks_query_retriever: The retriever instance to use for document retrieval.

    Returns:
        A list containing the page content of relevant documents.
    """

    # Retrieve relevant documents for the given question using the new invoke method
    docs = chunks_query_retriever.invoke(question)

    # Extract document content as a list
...
async def retry_with_exponential_backoff(coroutine, max_retries=5):
    """
    Retries a coroutine using exponential backoff upon encountering a LangChainException.
...
def get_langchain_embedding_provider(provider: EmbeddingProvider, model_id: str = None):
    ...
    """
    if provider == EmbeddingProvider.GOOGLE_GENAI:
        return GoogleGenerativeAIEmbeddings(model=model_id or "models/embedding-001")
    elif provider == EmbeddingProvider.HUGGINGFACE:
        try:
            from langchain_huggingface import HuggingFaceEmbeddings
            return HuggingFaceEmbeddings(model_name=model_id or "sentence-transformers/all-MiniLM-L6-v2")
        except ImportError:
            raise ImportError("Please install langchain-huggingface: pip install langchain-huggingface")
    elif provider == EmbeddingProvider.OLLAMA:
        try:
            from langchain_ollama import OllamaEmbeddings
            return OllamaEmbeddings(model=model_id or "nomic-embed-text")
        except ImportError:
        ...
        try:
            ...
            raise ImportError("Please install langchain-cohere: pip install langchain-cohere")
        ...
        try:
            ...
            raise ImportError("Please install langchain-aws: pip install langchain-aws")
    else:
...
def get_default_embeddings():
    """
    Returns the default free embedding provider (Google Gemini).
    
    Returns:
        GoogleGenerativeAIEmbeddings: A Google Gemini embedding provider instance.
    """
