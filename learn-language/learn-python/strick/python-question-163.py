...
def longest_palindromic_subsequence(s):
    ...
    # dp[i][j] represents the length of the LPS in the substring s[i:j+1].
    ...
    # Iterate through substrings of increasing lengths (length 2 to n).
    for length in range(2, n + 1):
        for i in range(n - length + 1):
            ...
            if s[i] == s[j]:
                dp[i][j] = 2 + (dp[i+1][j-1] if i+1 <= j-1 else 0)  # Added boundary check

            # Otherwise, the LPS length is the maximum of the LPS lengths
            # of the substrings obtained by excluding either the first or the last character.
            else:
                dp[i][j] = max(dp[i+1][j] if i+1 < n else 0, dp[i][j-1] if j-1 >= 0 else 0) # Added boundary check
