...
class KthLargest:
    ...
    def __init__(self, k: int, nums: list[int]):
        ...
        for num in nums:
            self.add(num)  # Add initial numbers to the heap

    def add(self, val: int) -> int:
        """
        Adds a new element 'val' to the stream and returns the k-th largest element.
        """
        if len(self.heap) < self.k:
            heapq.heappush(self.heap, val)  # If heap is not full, simply add
        elif val > self.heap[0]:
            heapq.heapreplace(self.heap, val)  # If val is larger than smallest in heap, replace

        return self.heap[0]  # The k-th largest is always the root of the min-heap
