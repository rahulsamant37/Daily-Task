    ...
    def longest_palindromic_subsequence(s):
        ...
        for length in range(2, n + 1):
            for i in range(n - length + 1):
                ...
                # the length of the LPS is 2 + the length of the LPS of the substring without those characters.
                if s[i] == s[j]:
                    dp[i][j] = 2 + dp[i + 1][j - 1] if i+1 <= j-1 else 2
                # Otherwise, the length of the LPS is the maximum of the lengths of the LPS of the substrings
                # without either the first or the last character.
                else:
