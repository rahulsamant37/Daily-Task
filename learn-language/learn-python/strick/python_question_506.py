...
Input: nums = [1, 5, 2, 3, 4], k = 1
Output: 3
Explanation: The longest increasing subsequence with a difference of at most 1 is [2, 3, 4].
'''
...
def longest_increasing_subsequence_with_k_difference(nums, k):
    ...
    Args:
        ...
        k: An integer representing the maximum allowed difference between
           consecutive elements in the subsequence.

    Returns:
        The length of the longest increasing subsequence with a difference of at
        most k.
    """
    ...
    for i in range(1, n):
        for j in range(i):
            ...
            if nums[i] > nums[j] and nums[i] - nums[j] <= k:
                # If so, update dp[i] if a longer subsequence can be formed
...
def test_solution():
    ...
    assert longest_increasing_subsequence_with_k_difference([1, 5, 2, 3, 4], 2) == 4
    assert longest_increasing_subsequence_with_k_difference([1, 5, 2, 3, 4], 1) == 3
