...
*   int add(int val) Appends the integer val to the stream and returns the element representing the kth largest element in the stream.  If the stream has less than k elements, return the smallest element in the stream.
...
class KthLargest:
    ...
    def __init__(self, k: int, nums: list[int]):
        ...
        Args:
            k: The integer representing the kth largest element to find.
    ...
    def add(self, val: int) -> int:
        ...
        if len(self.heap) < self.k:
            heapq.heappush(self.heap, val)  # Add to the heap if it has fewer than k elements.
        elif val > self.heap[0]:
            heapq.heapreplace(self.heap, val)  # Replace the smallest element if val is larger.

        if len(self.heap) < self.k:
            if len(self.heap) == 0:
                return -float('inf') #Return negative infinity when heap is empty
            else:
                return heapq.nsmallest(1, self.heap)[0] #Return smallest element in the stream

        return self.heap[0]  # The kth largest element is always at the root of the min-heap.
