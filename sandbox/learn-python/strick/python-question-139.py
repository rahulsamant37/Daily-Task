...
def longest_palindromic_subsequence(s):
    ...
    for length in range(2, n + 1):
        ...
        for i in range(n - length + 1):
            ...
            # then the longest palindromic subsequence is 2 + the longest palindromic subsequence
            # of the substring between them
            if s[i] == s[j]:
                dp[i][j] = 2 + (dp[i + 1][j - 1] if i + 1 <= j - 1 else 0)
            # Otherwise, the longest palindromic subsequence is the maximum of the longest palindromic subsequences
            # of the substrings excluding the start or the end
            else:
