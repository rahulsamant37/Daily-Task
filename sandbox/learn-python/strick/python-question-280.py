...
class KthLargest:
    ...
    def __init__(self, k: int, nums: list[int]):
        ...
        """
        self.k = k
        self.heap = nums  # Use a list initially, will be heapified later
        ...
        while len(self.heap) > k:
            heapq.heappop(self.heap)  # Remove the smallest element if heap size exceeds k
    ...
    def add(self, val: int) -> int:
        ...
        Returns:
            The kth largest element in the stream after adding the new value.
        """
        if len(self.heap) < self.k:
            heapq.heappush(self.heap, val)  # Add the value to the heap if size < k
