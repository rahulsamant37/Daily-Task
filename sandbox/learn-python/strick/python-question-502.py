...
class KthLargest:
    ...
    def add(self, val: int) -> int:
        ...
        if len(self.heap) < self.k:
            heapq.heappush(self.heap, val)  # Add the new element if the heap size is less than k
        elif val > self.heap[0]:  # If the new element is larger than the smallest element in the heap
            heapq.heapreplace(self.heap, val)  # Replace the smallest element with the new element
