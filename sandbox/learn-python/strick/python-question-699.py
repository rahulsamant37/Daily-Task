...
def longest_palindrome(s):
    ...
    The approach uses dynamic programming. A 2D table `dp` is created where
    ...
    - Two consecutive characters are palindromes if they are equal (dp[i][i+1] = True if s[i] == s[i+1])

    For substrings of length 3 or more, dp[i][j] is True if s[i] == s[j] and dp[i+1][j-1] is True.

    The longest palindromic substring is tracked by storing its start index and length.
    """
    n = len(s)
    if n < 2:
        return s
