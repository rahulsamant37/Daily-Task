...
def longest_palindromic_subsequence(s):
    ...
    # dp[i][j] will store the length of the LPS of s[i:j+1].
    ...
    for length in range(2, n + 1):
        # Iterate over the starting indices of substrings.
        for i in range(n - length + 1):
            ...
            if s[i] == s[j]:
                dp[i][j] = 2 + dp[i + 1][j - 1]
            # Otherwise, the LPS length is the maximum of the LPS lengths of the two
