...
'''
Design a class to find the kth largest element in a stream.  Essentially, you need to maintain the k largest elements seen so far.
...
class KthLargest:
    ...
    def __init__(self, k: int, nums: list[int]):
        ...
        for num in nums:
            self.add(num)  # Initialize heap with initial nums
    ...
    def add(self, val: int) -> int:
        ...
        if len(self.heap) < self.k:
            heapq.heappush(self.heap, val)  # Add to heap if heap size < k
        elif val > self.heap[0]:
            heapq.heapreplace(self.heap, val)  # Replace smallest in heap if val is larger
...
def test_solution():
    ...
    # Test case 4: k is larger than initial nums length
    kthLargest = KthLargest(4, [1,2])
