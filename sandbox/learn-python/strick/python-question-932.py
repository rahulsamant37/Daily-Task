...
'''
Given an array of integers `nums` and an integer `k`, find the length of the longest increasing subsequence (LIS) where the difference between consecutive elements in the subsequence is at most `k`.  In other words, for any two consecutive elements `nums[i]` and `nums[j]` in the subsequence (where `i < j`), it must be true that `nums[j] - nums[i] <= k`.

Example:
Input: nums = [1, 3, 2, 4, 5], k = 1
Output: 4
Explanation: The longest increasing subsequence with the given constraint is [1, 2, 3, 4] or [2,3,4,5]. The length is 4.

Input: nums = [1, 5, 2, 4, 3], k = 2
Output: 3
Explanation: The longest increasing subsequence with the given constraint is [1, 2, 3]. The length is 3.
'''
